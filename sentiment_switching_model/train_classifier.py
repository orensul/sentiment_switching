...
from sentiment_switching_model.models.text_classifier import TextCNN
from sentiment_switching_model.utils import data_processor, log_initializer, tf_session_helper

logger = None


def train(options, word_index, x_train, y_train, x_dev, y_dev):
    sess = tf_session_helper.get_tensorflow_session()
    with sess.as_default():
        cnn = TextCNN(sequence_length=x_train.shape[1], num_classes=y_train.shape[1], vocab_size=options['vocab_size'],
                      embedding_size=128, filter_sizes=list(map(int, [3, 4, 5])), num_filters=128, l2_reg_lambda=0.0)
        ...
        # Generate batches
        batches = data_processor.batch_iter(list(zip(x_train, y_train)), mconf.batch_size, options['training_epochs'])
...
def split_samples(y, x_shuffled, y_shuffled):
...
def train_classifier_model(options):
    ...
    x = np.asarray(x)
    [y, _] = data_processor.get_labels(options['label_file_path'], True, global_config.classifier_save_directory)
    x_shuffled, y_shuffled = get_shuffle_indices(x, y)
    x_train, x_dev, y_train, y_dev = split_samples(y, x_shuffled, y_shuffled)
    ...
    train(options, word_index, x_train, y_train, x_dev, y_dev)
